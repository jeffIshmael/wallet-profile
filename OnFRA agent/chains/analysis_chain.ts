...
export async function runAnalysisChain(
  ...
  if (apiKey && !isInactiveWallet) {
    try {
      ...
      if (!aiAttestation) {
        ...
        aiAttestation = `Chainalyse AI attests that wallet ${address} has a financial health score of ${healthData.financialHealthScore}% and reputation standing of ${reputationData.reputationScore}/100. Portfolio risk is classified as ${riskData.riskCategory} with stable monthly inflows estimated at $${incomeData.monthlyIncomeEstimateUsd} USD.`;
      }
    } catch (err) {
  ...
  if (!aiDashboardSummary || !aiAttestation) {
    ...
    if (isInactiveWallet) {
      ...
      aiAttestation = `Chainalyse AI attests that wallet ${address} currently shows no verified Celo transaction activity in the analyzed period. Financial health is ${health}/100 and reputation is ${rep}/100. Estimated borrowing capacity is ${loanData.safeLoanRange} until income and savings patterns are established onchain.`;
    } else {
