...
### Onfra — Onchain financial reputation for lenders
...
**What Onfra does**  
Onfra turns Celo wallet activity into lender-ready financial intelligence:
...
1. Lender requires a Onfra Financial Passport (`REP-{id}`) during loan onboarding for wallet-native borrowers.
...
Jeff Ishmael · [@onfra_xyz](https://x.com/onfra_xyz) · [GitHub](https://github.com/jeffIshmael/wallet-profile)
...
**What we'd like to explore:** integrating Onfra as a borrower verification step before loan approval — your users connect their wallet (free), generate a verified passport (`REP-{id}`), and you verify it onchain before extending credit. We handle the intelligence layer; you keep the lending relationship.

**Why now:** Onfra is live on Celo mainnet with onchain attestations, an ERC-8004 AI agent ([OnFRA #9219](https://8004scan.io/agents/celo/9219)), and x402 micropayments. Demo: [youtu.be/7WC3lD5dDj4](https://youtu.be/7WC3lD5dDj4).
...
Best,  
Jeff Ishmael  
Onfra · [@onfra_xyz](https://x.com/onfra_xyz)
...
**Partnership angle:** Pretium refers users who need credit to Onfra; they generate a verified financial passport (`REP-{id}`) that a lender can verify onchain. Pretium doesn't need to become a lender — you become the distribution channel for a verification layer your payment users will need.
...
**Proposal:** explore Onfra as a pre-borrow verification layer for Morpho markets on Celo. Borrowers submit a `REP-{id}` passport; curators (e.g. Feather) or integrators verify via API or onchain before loan approval.
...
Onfra analyzes Celo wallet activity and produces lender-ready scores, income estimates, and verified financial passports with onchain attestation (`REP-{id}`). Borrowers analyze their own wallet for free; verified passports cost 0.10 USDT.
