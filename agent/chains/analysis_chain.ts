...
export async function runAnalysisChain(walletAddress: string, blockHeight?: number): Promise<DashboardOutput> {
  ...
  // 2. Fetch raw data (fast overview metadata)
  const rawDataJson = await fetchOnchainData.invoke({ walletAddress });
  const rawData = JSON.parse(rawDataJson);

  // 3. Fetch 3-month transaction flow statement (Lipa Mdogo Mdogo default)
  ...
  // Merge the statement metrics and transaction list into rawData
  ...
  const combinedDataJson = JSON.stringify(rawData);

  // 4. Compute scores and breakdowns in sequence (inputs are passed down)
  const healthJson = await computeFinancialHealth.invoke({ onchainDataJson: combinedDataJson });
  const healthData = JSON.parse(healthJson);

  const reputationJson = await computeReputationScore.invoke({ onchainDataJson: combinedDataJson });
  const reputationData = JSON.parse(reputationJson);

  const riskJson = await riskExposure.invoke({ onchainDataJson: combinedDataJson });
  const riskData = JSON.parse(riskJson);

  const incomeJson = await incomeStability.invoke({ onchainDataJson: combinedDataJson });
  ...
  if (apiKey) {
    try {
      ...
      if (!aiAttestation) {
        ...
        aiAttestation = `Wallet Profile AI attests that wallet ${address} has a financial health score of ${healthData.financialHealthScore}% and reputation standing of ${reputationData.reputationScore}/100. Portfolio risk is classified as ${riskData.riskCategory} with stable monthly inflows estimated at $${incomeData.monthlyIncomeEstimateUsd} USD.`;
      }
    } catch (err) {
